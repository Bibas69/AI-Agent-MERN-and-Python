...
const SmartNotification = () => {
    ...
    const fetchTasks = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `${getBackendUrl()}/api/task/getTaskBySingleDate`,
          {
            params: {
              uid: currentUser.uid,
              date: date,
  ...
  const currentTaskTime = currentTaskDate.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  const next = () => {
    setCurrentIndex(
      currentIndex + 1 < tasks.length ? currentIndex + 1 : 0
    );
  };

  /* ======================================================
     NEW AUTO-RESPONSIVE WRAPPER (MATCHES YOUR OTHER COMPONENTS)
     ====================================================== */

  const wrapperClass =
    "w-full max-w-[360px] sm:max-w-[420px] md:max-w-[480px] lg:max-w-[520px] xl:max-w-[560px] " +
    "bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 rounded-xl " +
    "border border-zinc-700 shadow-lg shadow-black/40 p-4 sm:p-6";
